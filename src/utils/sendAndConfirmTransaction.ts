...
const watchBlockHeight = async (
	...
	while (new Date().getTime() - startTime < MAX_CONFIRMATION_TIME && !abortSignal.aborted) {
		...
		if (blockHeight > txValidUntilBlockHeight) {
			return TransactionError.BLOCK_HEIGHT_EXCEEDED
		}
...
export type ErrorResponse = {
	success: false
	err: null | TransactionError.BLOCK_HEIGHT_EXCEEDED
}
...
export const sendAndConfirmTransaction = async (
	...
	const blockHeightErr = TransactionError.BLOCK_HEIGHT_EXCEEDED
